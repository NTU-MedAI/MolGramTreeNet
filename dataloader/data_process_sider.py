...
class MolGraphDataset():
    def __init__(self, path, num_labels=12):
        df = pd.read_csv(path)
    ...
    def _calculate_class_weights(self):
        weights = []
        for i in range(self.targets_1.shape[1]):
            valid_labels = self.targets_1[~np.isnan(self.targets_1[:, i]), i]
    ...
    def get_smiles_augmentations(self, smiles, n=3):
        try:
