...
class MolGraphDataset():
    def __init__(self, path):
        ...
        n_cols = file.shape[1]
        self.header_cols = np.genfromtxt(path, delimiter=',', max_rows=1, dtype=np.str_, comments=None)
        self.target_names = [self.header_cols[-1]]  # 获取最后一列（Class）的列名

        # 读取数据部分
        self.smiles_1 = np.genfromtxt(path, delimiter=',', skip_header=1, usecols=[0], dtype=np.str_,
                                      comments=None)  # smiles在第0列
        self.targets_1 = np.genfromtxt(path, delimiter=',', skip_header=1, usecols=[1], dtype=np.int_,
                                       comments=None)  # Class在第1列
...
def molgraph_collate_fn(data):
    ...
    # For graph data, we need to handle variable-sized graphs
    x_list = []
    edge_index_list = []
    edge_attr_list = []
    batch_indices = []
    ...
    for i in range(batch_size):
        ...
        # Handle graph data
        x_list.append(x)
        edge_index_list.append(edge_index + num_nodes)  # Offset for batch
