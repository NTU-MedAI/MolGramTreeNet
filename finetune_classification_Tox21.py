...
from dataloader.data_process_Tox21 import MolGraphDataset, molgraph_collate_fn, suppress_output
...
def main():
    ...
    args.epochs = 80
    args.gpu = '0'
    args.lr = 8e-5 * len(args.gpu.split(','))
    args.bs = 32 * len(args.gpu.split(','))
    ...
    with suppress_output():
        full_dataset = MolGraphDataset('/home/ntu/Documents/zyk/MolGramTreeNet/data/tox21/raw/tox21.csv')
    ...
    lr_scheduler = opt.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', factor=0.7, patience=8, min_lr=1e-6)
    ...
    if not os.path.exists(args.save_path + 'tox21/'):
        os.makedirs(args.save_path + 'tox21/')
    ...
    torch.save(checkpoint, args.save_path + f'tox21/tox21_model.pt')
    log.logger.info('{} End Training (Time: {:.2f}h) {}'.format("=" * 20, (time() - t0) / 3600, "=" * 20))
    log.logger.info(f'Save the best model as tox21_model.pt.\n')
